function results = RunExperimentWithFixedTestSet(method, dataset, expParams)
%
% Run method on dataset with cross validation and obtain results with fixed
% test set
%
% Input(s)
...
%   dataset: dataset structure containing tvx, tvy, tsx, tsy matrices, some methods
...
% Department of Computer Engineering
    ...
    if notify
        fprintf('RunExperimentWithFixedTestSet: Experiment started. Method: %s, Dataset: %s, Problem Type: %s, CV: %dx%d\n', ...
            method.name, dataset.name, expParams.problemType, expParams.cvRunCount, expParams.cvFoldCount);
    end
    
    % Cross Validation
    N = size(dataset.tvx,1);
    for r = 1:expParams.cvRunCount
        cv = cvpartition(dataset.tvy, 'kfold', expParams.cvFoldCount);    
        ...
        for k = 1:expParams.cvFoldCount
            ...
            dataset.trainx = dataset.tvx(istrain,:);
            dataset.trainy = dataset.tvy(istrain,:);
            dataset.testx = dataset.tsx;
            dataset.testy = dataset.tsy;
